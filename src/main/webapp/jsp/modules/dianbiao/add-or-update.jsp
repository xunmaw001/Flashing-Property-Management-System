            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑售电</li>
                    </ol>
                </div>
            </div>
        </div>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="dianbiaoNumber" name="dianbiaoNumber" class="form-control"
                                               onchange="dianbiaoNumberChickValue(this)"  placeholder="电表余额">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("dianbiao/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('adddianbiao');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                dianbiaoName: "required",
                dianbiaoNumber: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                dianbiaoName: "电表编号不能为空",
                dianbiaoNumber: "电表余额不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var adddianbiao = window.sessionStorage.getItem("adddianbiao");
        if (adddianbiao != null && adddianbiao != "" && adddianbiao != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("dianbiao/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
        if(window.sessionStorage.getItem('adddianbiao') != null && window.sessionStorage.getItem('adddianbiao') !=""){//新增查询,修改不查
            // 查询当前登录账户的信息
            var userId = window.sessionStorage.getItem('userId');
