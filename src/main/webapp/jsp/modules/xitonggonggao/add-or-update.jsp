...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("xitonggonggao/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addxitonggonggao');
    ...
    // 获取当前详情
    function getDetails() {
        var addxitonggonggao = window.sessionStorage.getItem("addxitonggonggao");
        if (addxitonggonggao != null && addxitonggonggao != "" && addxitonggonggao != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("xitonggonggao/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#leixing").val(ruleForm.leixing);
        $("#neirong").val(ruleForm.neirong);
        $("#riqi").val(ruleForm.riqi);
